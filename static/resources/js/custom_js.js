...
function addRow()
{
	...
	select.setAttribute("onchange", "getData(this.value, "+rNum+")");
...
{
	console.log(val, id);
	var key_1 = document.getElementById("cell_"+id+"0").childNodes[0].value;
	console.log(key_1);
	...
	for(i = 1; i < rNum; i++)
	{
		...
		for(j = 0; j < 2; j++)
		{
			...
			if(j == 0)
			{
				...
				td.innerHTML = document.getElementById("cell_"+i+"0").childNodes[0].value;
			}
			else if(j == 1)
			{
				// td.setAttribute("style", "background: blue;")
